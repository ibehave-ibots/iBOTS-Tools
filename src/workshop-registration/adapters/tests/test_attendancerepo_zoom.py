...
from external.zoom_api import ZoomAttendee, OAuthGetToken, get_attendees
...
@pytest.mark.slow
def test_zoom_attendance_repo_returns_correct_attendees_for_a_given_zoom_workshop():
    load_dotenv()
    oauth = OAuthGetToken(
    ...
    repo = ZoomAttendanceRepo(oauth_get_token=oauth, get_attendees=get_attendees)
    workshop_id = "81504158493"

    attendance_records = repo.get_attendance_records(workshop_id=workshop_id)
    assert len(attendance_records) == 6
    assert attendance_records[0].workshop_id == workshop_id
